...
class EspCamWidget(QtWidgets.QWidget):
    ...
    def populate_ui_ctrl(self):
        ...
        self.esp32_button = QtWidgets.QPushButton("Start")
    ...
    def grab_image(self):
        # grab image from images folder is there are exist
        files = os.listdir("./images")
        print(files)

        # Check if there are any files in the folder
        if not files:
            self.label_button.setText("-END-")
            self.esp32_button.clicked.disconnect()
            self.esp32_button.repaint()
            return
        self.imgPath = os.path.join("./images", files[0])
        print(self.imgPath)
    ...
    def upload_data(self):
        ...
        #? move image to done
        os.rename(self.imgPath, self.imgPath.replace("images", "doneImages"))
        self.grab_image()
